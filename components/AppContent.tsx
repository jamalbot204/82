...
const AppContent: React.FC = memo(() => {
  const { currentChatSession } = useActiveChatStore(useShallow(state => ({
    currentChatSession: state.currentChatSession
  })));
  const { isLoadingData } = useChatListStore(useShallow(state => ({
  ...
  return (
    <div className="flex h-screen antialiased text-aurora-textPrimary bg-transparent overflow-hidden">
      
        <div className={`fixed inset-y-0 left-0 z-30 transform transition-transform duration-300 ease-apple ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} w-72`}>
          <Sidebar />
        </div>

        {isSidebarOpen && <div className="fixed inset-0 z-20 bg-black/50 backdrop-blur-sm md:hidden" onClick={closeSidebar} aria-hidden="true" />}
        
        <main 
          className={`relative z-10 flex-1 flex flex-col overflow-y-auto transition-all duration-300 ease-in-out ${isSidebarOpen ? 'md:ml-72' : ''} ${isAudioBarVisible ? 'pt-[76px]' : ''}`}
          onClick={() => {
            // Close sidebar when clicking on the chat area if it's open
            // This primarily affects desktop view as mobile has a backdrop overlay
