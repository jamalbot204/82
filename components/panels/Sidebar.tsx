...
const Sidebar: React.FC = memo(() => {
  const { editingTitleInfo, startEditingTitle, setEditingTitleValue, cancelEditingTitle, saveChatTitle } = useChatTitleStore(useShallow(state => ({
    editingTitleInfo: state.editingTitleInfo,
    startEditingTitle: state.startEditingTitle,
    setEditingTitleValue: state.setEditingTitleValue,
    cancelEditingTitle: state.cancelEditingTitle,
    saveChatTitle: state.saveChatTitle
  })));

  const { currentChatId, selectChat, showAdvancedDataTools, isCharacterModeActive } = useActiveChatStore(useShallow(state => ({
  ...
  const { chatHistory, createNewChat, deleteChat, duplicateChat } = useChatListStore(useShallow(state => ({
    chatHistory: state.chatHistory,
    createNewChat: state.createNewChat,
    deleteChat: state.deleteChat,
    duplicateChat: state.duplicateChat
  })));

  const { handleEmbedSelectedChats, handleResetEmbedFlags } = useDataStore(useShallow(state => ({
    handleEmbedSelectedChats: state.handleEmbedSelectedChats,
    handleResetEmbedFlags: state.handleResetEmbedFlags
  })));

  const { handleImportAll } = useImportStore(useShallow(state => ({
    handleImportAll: state.handleImportAll
  })));
  
  const { openSettingsPanel, openExportConfigurationModal, openTelegramImportModal, openTextExportModal } = useSettingsUI(useShallow(state => ({
    openSettingsPanel: state.openSettingsPanel,
    openExportConfigurationModal: state.openExportConfigurationModal,
    openTelegramImportModal: state.openTelegramImportModal,
    openTextExportModal: state.openTextExportModal
  })));

  const { requestDeleteChatConfirmation, requestDeleteHistoryConfirmation } = useConfirmationUI(useShallow(state => ({
    requestDeleteChatConfirmation: state.requestDeleteChatConfirmation,
    requestDeleteHistoryConfirmation: state.requestDeleteHistoryConfirmation
  })));
  
  const { layoutDirection, toggleLayoutDirection, theme, toggleTheme, toggleLanguage, isSidebarOpen } = useGlobalUiStore(useShallow(state => ({
  ...
  const { toggleCharacterMode } = useCharacterStore(useShallow(state => ({
    toggleCharacterMode: state.toggleCharacterMode
  })));

  const { isHistorySelectionModeActive, toggleHistorySelectionMode, selectedChatIds, toggleChatSelection, selectAllChats, deselectAllChats } = useHistorySelectionStore(useShallow(state => ({
    isHistorySelectionModeActive: state.isHistorySelectionModeActive,
    toggleHistorySelectionMode: state.toggleHistorySelectionMode,
    selectedChatIds: state.selectedChatIds,
    toggleChatSelection: state.toggleChatSelection,
    selectAllChats: state.selectAllChats,
    deselectAllChats: state.deselectAllChats
  })));
  ...
  return (
    <div className={`w-72 aurora-panel h-full flex flex-col border-r border-aurora-border gpu-accelerated`}>
      <div className="p-4 flex-shrink-0 z-10">
        <div className="p-3 rounded-xl bg-black/20 border border-aurora-border backdrop-blur-sm flex justify-between items-center shadow-sm">
            <h1 className="text-lg font-bold text-aurora-textPrimary flex items-baseline tracking-tight">
            {APP_TITLE}
      ...
      <div className="px-4 space-y-3 flex-shrink-0">
        {isHistorySelectionModeActive ? (
           <div className="grid grid-cols-2 gap-2">
              <button onClick={handleSelectAll} className="flex items-center justify-center px-3 py-2 text-xs font-medium text-aurora-textSecondary bg-white/5 rounded-lg border border-white/5 hover:bg-white/10 transition-all">{t.selectAll}</button>
              <button onClick={deselectAllChats} className="flex items-center justify-center px-3 py-2 text-xs font-medium text-aurora-textSecondary bg-white/5 rounded-lg border border-white/5 hover:bg-white/10 transition-all">{t.deselectAll}</button>
              ...
              <button onClick={toggleHistorySelectionMode} className="col-span-2 flex items-center justify-center px-3 py-2 text-xs font-medium text-aurora-textSecondary bg-white/5 rounded-lg border border-white/5 hover:bg-white/10 transition-all">{t.cancelSelection}</button>
           </div>
        ) : (
          <>
            <div className="flex space-x-2">
                ...
                className="flex-1 flex items-center justify-center px-4 py-2.5 text-sm font-bold text-white bg-aurora-accentPrimary rounded-lg shadow-lg shadow-blue-900/30 hover:shadow-blue-600/40 hover:scale-[1.02] active:scale-95 transition-all duration-200"
                >
                ...
                <button
                    ...
                    className={`p-2.5 text-sm font-medium rounded-lg transition-all duration-200 focus:outline-none border
                                ${isCharacterModeActive 
                                    ...
                                    : 'bg-white/5 text-aurora-textSecondary border-white/5 hover:bg-white/10 hover:border-white/10'}
            ...
            <div className="grid grid-cols-2 gap-2">
                <button onClick={openExportConfigurationModal} title={t.export} className="flex items-center justify-center px-3 py-2 text-xs font-medium text-aurora-textSecondary bg-white/5 rounded-lg border border-white/5 hover:bg-white/10 transition-all"><ExportIcon className="w-3.5 h-3.5 mr-1.5 ltr:mr-1.5 rtl:ml-1.5 rtl:mr-0" />{t.export}</button>
                <button onClick={handleImportAll} title={t.import} className="flex items-center justify-center px-3 py-2 text-xs font-medium text-aurora-textSecondary bg-white/5 rounded-lg border border-white/5 hover:bg-white/10 transition-all"><ImportIcon className="w-3.5 h-3.5 mr-1.5 ltr:mr-1.5 rtl:ml-1.5 rtl:mr-0" />{t.import}</button>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-2 space-y-2 custom-scrollbar">
        <div className="flex items-center justify-between mb-2 mt-2">
            <h2 className="text-[10px] font-bold text-aurora-textSecondary uppercase tracking-widest opacity-70 pl-1">{t.history}</h2>
            <button 
                onClick={toggleHistorySelectionMode}
                className={`p-1 rounded-md transition-colors ${isHistorySelectionModeActive ? 'text-aurora-accentPrimary bg-blue-500/10' : 'text-gray-500 hover:text-aurora-textPrimary hover:bg-white/5'}`}
                title={t.select}
            >
        ...
        {chatHistory.map(session => {
            ...
            let textClass = 'text-aurora-textSecondary';
            ...
            if (isActive) {
                if (isCharMode) {
                    ...
                    borderClass = 'border-l-aurora-accentPrimary';
                    bgClass = 'bg-gradient-to-r from-aurora-accentPrimary/10 to-transparent shadow-[0_4px_20px_-5px_rgba(59,130,246,0.3)]';
                    textClass = 'text-aurora-textPrimary';
                }
            } else if (isSelected) {
                ...
                <div className="flex items-center overflow-hidden flex-grow">
                    ...
                    {isEditing ? (
                        <input
                            ...
                            className="text-sm bg-black/50 text-aurora-textPrimary rounded-md px-2 py-1 w-full focus:outline-none focus:ring-1 focus:ring-aurora-accentPrimary border border-white/10"
      ...
      <div className="p-4 pt-2 border-t border-aurora-border">
        <button
          onClick={openSettingsPanel}
          className="w-full flex items-center justify-center px-4 py-3 text-sm font-medium text-aurora-textSecondary bg-white/5 rounded-xl border border-white/5 hover:bg-white/10 hover:border-white/10 hover:text-white transition-all shadow-sm"
        >
