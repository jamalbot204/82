...
const ChatMessageList = memo(forwardRef<ChatMessageListHandles, ChatMessageListProps>(({ onEnterReadMode }, ref) => {
    ...
    const handleScroll = useCallback(() => {
        const now = Date.now();
        if (now - lastScrollTimeRef.current < 150) return; // 150ms throttle
        ...
        if (messageListRef.current) {
            ...
            // Only update state if values actually change to prevent re-renders
            setShowScrollTop(prev => prev !== (scrollTop > 500) ? (scrollTop > 500) : prev);
            setShowScrollBottom(prev => prev !== (distanceFromBottom > 500) ? (distanceFromBottom > 500) : prev);
        }
    }, []);
    ...
    return (
        <div ref={messageListRef} onScroll={handleScroll} className={`flex-1 p-4 sm:p-6 overflow-y-auto relative`} role="log" aria-live="polite">
            ...
            {currentChatSession ? (
                ...
                ) : ( <div className="text-center text-aurora-textSecondary italic mt-10">{isCharacterMode && characters.length === 0 ? "Add some characters and start the scene!" : (isCharacterMode ? "Select a character to speak." : t.noChats)}</div>)
            ) : ( <div className="text-center text-aurora-textSecondary italic mt-10">{t.noChats}</div>)}
            ...
            <div className={`sticky bottom-0 w-full flex justify-end px-1 z-50 pointer-events-none transition-opacity duration-300 -mt-10 ${showScrollBottom ? 'opacity-100' : 'opacity-0'}`}>
                <button
                    ...
                    className="p-1.5 rounded-full bg-aurora-accentPrimary/90 text-white hover:bg-aurora-accentPrimary backdrop-blur-md border border-white/10 shadow-lg pointer-events-auto transform -translate-y-1"
