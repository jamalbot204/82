...
const ChatHeader: React.FC<ChatHeaderProps> = memo(({ isReorderingActive, toggleReordering, onJumpToMessage }) => {
    ...
    return (
        <header className="sticky top-0 z-20 flex items-center justify-between px-4 py-3 bg-aurora-bg/60 backdrop-blur-2xl border-b border-aurora-border shadow-sm transition-all duration-300">
                                            ...
                                            <button
                                                ...
                                                className={`text-left px-3 py-2 text-xs rounded-md transition-colors flex justify-between items-center ${
                                                    currentChatSession.model === def.id 
                                                        ? 'bg-aurora-accentPrimary text-white font-medium shadow-md' 
