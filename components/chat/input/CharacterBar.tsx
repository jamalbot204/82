            ...
            <div className="flex flex-wrap gap-2">
                {characters.map((char) => (
                    <button 
                        ...
                        className={`px-3 py-1.5 text-xs font-medium bg-aurora-accentSecondary text-white rounded-lg disabled:opacity-50 transition-all duration-200 ease-out hover:scale-105 hover:shadow-[0_0_10px_2px_rgba(156,51,245,0.4)] ${isReorderingActive ? 'cursor-grab hover:ring-2 hover:ring-purple-400' : 'disabled:cursor-not-allowed'} ${draggedCharRef.current?.id === char.id ? 'opacity-50 ring-2 ring-blue-500' : ''}`}
                    >
                        {char.name}
                    </button>
                ))}
