...
const MessageItemComponent: React.FC<MessageItemProps> = ({ message, canRegenerateFollowingAI, highlightTerm, onEnterReadMode, isContentExpanded, isThoughtsExpanded, onToggleExpansion, isLatestMemoryUpdate }) => {
  ...
  const { currentChatSession, activeStreamText } = useActiveChatStore(useShallow(state => ({
      currentChatSession: state.currentChatSession,
      activeStreamText: message.isStreaming ? state.activeStreamText : null
  })));
  ...
  const { cleanContent, choices } = useMemo(() => {
      // Phase 3: Use activeStreamText if available and message is streaming
      const contentToUse = (message.isStreaming && activeStreamText) ? activeStreamText : message.content;
      
      if (!isInteractiveChoicesEnabled || !contentToUse) {
          return { cleanContent: contentToUse, choices: [] };
      }
      return parseInteractiveChoices(contentToUse);
  }, [message.content, message.isStreaming, activeStreamText, isInteractiveChoicesEnabled]);
  ...
  if (message.isTimeMarker) {
      ...
      return (
          <div className="flex justify-center items-center my-6 w-full opacity-70 select-none pointer-events-none animate-message-enter">
              <span className="text-[10px] sm:text-xs font-medium text-aurora-textSecondary bg-black/20 px-3 py-1 rounded-full border border-white/5 flex items-center shadow-sm">
      ...
      return (
          ...
          >
              <div className="relative flex items-center">
                  <div 
                      className={`flex items-center justify-center p-1.5 rounded-full bg-white/5 border border-white/5 transition-all cursor-help ${isLatestMemoryUpdate ? 'border-cyan-500/50 shadow-[0_0_10px_rgba(6,182,212,0.3)] bg-cyan-900/20' : 'hover:border-aurora-accentPrimary hover:bg-aurora-accentPrimary/10'}`}
                  >
                      {isLatestMemoryUpdate ? (
                          <BrainIcon className="w-4 h-4 text-cyan-400 animate-pulse" />
                      ) : (
                          <ArrowPathIcon className="w-3.5 h-3.5 text-gray-500 group-hover/divider:text-aurora-accentPrimary transition-colors" />
                      )}
                  </div>
  ...
  let bubbleClasses = '';
  if (isUser) {
      bubbleClasses = 'bg-aurora-msgUserBg text-aurora-textOnAccent rounded-2xl rounded-br-sm shadow-[0_4px_20px_rgba(0,0,0,0.15)] border border-aurora-border backdrop-blur-md hover:shadow-[0_4px_25px_rgba(0,0,0,0.2)] transition-shadow duration-300';
  } else if (isError) {
      bubbleClasses = 'bg-red-900/30 backdrop-blur-xl border border-red-500/50 text-red-100 shadow-lg shadow-red-900/20 rounded-2xl rounded-bl-sm transition-shadow duration-300';
  } else {
      bubbleClasses = 'bg-aurora-msgAiBg backdrop-blur-xl border border-aurora-border text-aurora-textOnSurface rounded-2xl rounded-bl-sm shadow-[0_4px_20px_rgba(0,0,0,0.1)] hover:shadow-[0_4px_25px_rgba(0,0,0,0.15)] transition-shadow duration-300';
  }
      ...
      {!isUser && isSelectionModeActive && (
          <div className="flex-shrink-0 self-center px-2 flex items-center space-x-2">
              ...
              <input 
                ...
                className="w-4 h-4 text-aurora-accentPrimary bg-black/30 border-white/20 rounded focus:ring-aurora-accentPrimary focus:ring-offset-black cursor-pointer pointer-events-none" 
...
export default memo(MessageItemComponent, (prevProps, nextProps) => {
    // Strict Memoization Logic
    // Returns true if props are equal (skip re-render)
    // Returns false if props are different (trigger re-render)
    ...
    const isMessageEqual = 
        ...
        prevProps.message.errorType === nextProps.message.errorType &&
        // Deep check for attachments length to catch updates
    ...
    const isStateEqual = 
        prevProps.isContentExpanded === nextProps.isContentExpanded &&
        prevProps.isThoughtsExpanded === nextProps.isThoughtsExpanded &&
        prevProps.isLatestMemoryUpdate === nextProps.isLatestMemoryUpdate &&
        prevProps.highlightTerm === nextProps.highlightTerm &&
        prevProps.canRegenerateFollowingAI === nextProps.canRegenerateFollowingAI;

    // We can't easily check isSelectionModeActive from props since it comes from store in the component
    // However, the component subscribes to the store, so it will re-render if store changes.
    // The props passed from parent (ChatMessageList) are what we are comparing here.
