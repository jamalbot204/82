...
const DropdownMenuItem: React.FC<{
    ...
    <button role="menuitem" disabled={disabled} title={label} aria-label={label} className={`w-auto p-2 text-sm flex items-center justify-center rounded-md transition-all ${ disabled ? 'text-gray-500 cursor-not-allowed' : `text-aurora-textSecondary hover:text-aurora-textPrimary ${hoverGlowClassName || 'hover:bg-white/10'} ${className || ''}`}`} onMouseDown={() => { if (!disabled) onClick(); }} onTouchStart={() => { if (!disabled) onClick(); }} onClick={(e) => { e.preventDefault(); }}>
