...
(Proceed with the user's next message below)`;
        ...
        const handleStreamUpdate = (fullRawText: string) => {
            ...
            // Phase 3 Optimization: Isolate streaming state from main history to prevent deep re-renders
            useActiveChatStore.getState().setActiveStreamText(visibleText);
        };
        ...
        const handleFinalResponseSuccess = async (responseData: FullResponseData) => {
            ...
            useActiveChatStore.getState().setActiveStreamText(null);
