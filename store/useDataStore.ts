...
let updateMessagesDebounceTimer: any = null;
...
export const useDataStore = create<DataStoreState>((set, get) => ({
  ...
  updateMessages: async (chatId, newMessages) => { 
      // Phase 3: Debounce IndexedDB Writes
      if (updateMessagesDebounceTimer) clearTimeout(updateMessagesDebounceTimer);
      
      updateMessagesDebounceTimer = setTimeout(() => {
          dbService.updateMessagesInDB(chatId, newMessages).catch(console.error);
          updateMessagesDebounceTimer = null;
      }, 1000); // 1s debounce
  },
