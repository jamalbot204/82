...
export const useFileHandler = () => {
    ...
    for (let i = 0; i < fileList.length; i++) {
      ...
      const processFile = async () => {
          try {
              ...
              if (fileTypeForApp === 'image') {
                  // Phase 3: Client-Side Image Compression
                  resultDataUrl = await compressImage(file);
                  base64Data = resultDataUrl.split(',')[1];
              } else {
                  // Standard read for non-images
                  ...
                  if (!resultDataUrl || !resultDataUrl.startsWith('data:')) throw new Error("Invalid file data");
                  base64Data = resultDataUrl.split(',')[1];
              }

              updateFile(attachmentId, {
                  dataUrl: (fileTypeForApp === 'image' || fileTypeForApp === 'video') ? resultDataUrl : undefined,
              ...
              upload(attachmentId, file); // Refactored upload to be callable

          } catch (error) {
              updateFile(attachmentId, { error: "Failed to process file.", uploadState: 'error_client_read', isLoading: false });
          }
      };
  ...
  // Helper for upload logic (extracted from original reader.onload)
  const upload = async (attachmentId: string, file: File) => {
        if (!useApiKeyStore.getState().activeApiKey?.value) {
        ...
        try {
            ...
            const result = await uploadFileViaApi(
                useApiKeyStore.getState().activeApiKey!.value, 
                file, 
                logApiRequest, 
